...
namespace NotesMarketPlace.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult finalLogin(User u1)
        {
            ...
            if (us != null)
            {
                var up = nmp.UserProfiles.Where(x => x.UserID == us.ID).FirstOrDefault();
                ...
                {
                    Session["profilephoto"] = up.Profile_Picture.ToString();
                    FormsAuthentication.SetAuthCookie(u1.EmailID, false);
                    if (us.RoleID == 2)
                    {
                        return RedirectToAction("adminDashboard", "Admin");
        ...
        public ActionResult dashboard()
        {
            ...
            int SoldNotes = nmp.Downloads.Where(x => x.Seller == usr.ID && x.IsSellerHasAllowedDownload == true).Count();
            decimal? MoneyEarned = nmp.Downloads.Where(x => x.Seller == usr.ID && x.IsSellerHasAllowedDownload == true).Select(x => x.PurchasePrice).Sum();
            int MyDownloads = nmp.Downloads.Where(x => x.Downloader == usr.ID).Count();
            int MyRejectedNote = nmp.SellerNotes.Where(x => x.SellerID == usr.ID && x.Status == 4).Count();
            int BuyerRequest = nmp.Downloads.Where(x => x.Seller == usr.ID && x.IsSellerHasAllowedDownload == false).Count();
            ViewBag.D1 = SoldNotes;
            ViewBag.D2 = MoneyEarned;
            ViewBag.D3 = MyDownloads;
            ViewBag.D4 = MyRejectedNote;
            ViewBag.D5 = BuyerRequest;
